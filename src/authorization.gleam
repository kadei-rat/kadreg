...
pub fn can_manage_member_details(
  ...
  {
    True -> Ok(Nil)
    False -> Error("User is not authorised to manage this member's details")
  }
}

pub fn can_list_members(session_data: SessionData) -> Result(Nil, String) {
  case can_manage_members(session_data.role) {
    True -> Ok(Nil)
    False -> Error("User is not authorised to list all members")
  }
}

pub fn can_create_members(session_data: SessionData) -> Result(Nil, String) {
  case can_manage_members(session_data.role) {
    True -> Ok(Nil)
    False -> Error("User is not authorised to create new members")
  }
}
